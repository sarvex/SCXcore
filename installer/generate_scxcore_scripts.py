...
def Get_sh_path(PF):
    return '#!/usr/bin/sh' if PF in ["SunOS", "HPUX"] else '#!/bin/sh'

def GenerateSetupScriptFile():
    with open(os.path.join(outputDir, 'scx_setup.sh'), 'w') as shfile:
...
def GenerateToolsSetupScriptFile():
    with open(os.path.join(outputDir, 'scx_setup_tools.sh'), 'w') as shfile:
        shfile.write('# Copyright (c) Microsoft Corporation.  All rights reserved.\n')
        # Configure script to not complain if environment variable isn't currently set
        shfile.write('set +u\n')
        if Variables["PF"] == 'MacOS':
...
def GenerateAdminToolScriptFile():
    with open(os.path.join(outputDir, 'scxadmin.sh'), 'w') as shfile:
        shfile.write( Get_sh_path(Variables["PF"]) )
        shfile.write( '\n\n' );
        shfile.write('# Copyright (c) Microsoft Corporation.  All rights reserved.\n\n')

        scxpath = "/usr/libexec" if Variables["PF"] == 'MacOS' else "/opt"
        shfile.write(f'. {scxpath}' + '/microsoft/scx/bin/tools/setup.sh\n')
        ...
        shfile.write(f'exec {scxpath}' + '/microsoft/scx/bin/tools/.scxadmin "$@"\n')

def GenerateSSLToolScriptFile():
    with open(os.path.join(outputDir, 'scxsslconfig.sh'), 'w') as shfile:
        shfile.write( Get_sh_path(Variables["PF"]) );
        shfile.write( '\n\n' );
        shfile.write('# Copyright (c) Microsoft Corporation.  All rights reserved.\n\n')
        ...
        shfile.write(f'. {scxpath}' + '/microsoft/scx/bin/tools/setup.sh\n')
        shfile.write(
            f'exec {scxpath}' + '/microsoft/scx/bin/tools/.scxsslconfig "$@"\n'
        )
